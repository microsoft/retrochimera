...
class EnsembleModel(BackwardReactionModel):
    """Wrapper for an ensemble model."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initializes the ensemble model wrapper."""
        ...
        for key in model_data:
            if key in kwargs:
                # If the ensemble constructor receives an argument whose name matches one of the
        ...
        # Having removed kwargs meant for constituent models we can now call our own `__init__`.
        super().__init__(*args, **kwargs)

        self.device = device
        self._init_from_dir(
            model_dir=Path(model_dir), model_data=model_data, model_kwargs=model_kwargs
        )
